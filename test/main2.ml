...
module type Testable = sig
  ...
  val tree : entry list -> string
end

module C : Testable = struct
  type hash = string
  type entry_kind = Node | Content
  type entry = { name : string; kind : entry_kind; hash : hash }

  let get_enc e =
    let { Irmin_tezos.Partition.encoding = s; Irmin_tezos.Partition.v = _ } =
      e
    in
    s
  ...
  let lift_kind =
    function
    | Node -> Irmin_tezos.Partition.Node
    | Content -> Irmin_tezos.Partition.Content
  let lift_entry e =
    let { name = s; kind = k; hash = h } = e in
    { Irmin_tezos.Partition.name = s; Irmin_tezos.Partition.kind = lift_kind k; Irmin_tezos.Partition.hash = h }
  
  let tree l = Irmin_tezos.Partition.partition (List.map lift_entry l) |> get_enc
end
 ...
 *)
   ...
   *)
  type entry_kind = C.entry_kind = Node | Content
  type entry = C.entry = { name : string; kind : entry_kind; hash : hash }
